...
// =================================================================================================
// This file is CUDA translation of the original file of the CLTune project, which loosely follows the Google C++ styleguide and uses
// a tab-size of two spaces and a max-width of 100 characters per line.
//
// Author: cedric.nugteren@surfsara.nl (Cedric Nugteren)
//
// This file contains an example CUDA kernel. This assumes that
...
inline __device__ void InitAccRegisters(float acc[WPTY][WPTX]) {
  #pragma unroll
...
#if LOCAL == 2
inline __device__ void LoadLocalFull(float *lmem, const int loff,
                          const  floatvec* src, const int goff,
...
#if LOCAL == 1
inline __device__ void LoadLocalPlusHalo(float *lmem, const int loff,
                              const  float* src, const int goff,
                              const int gid_x, const int gid_y, const int lid_x, const int lid_y) {
...
#if LOCAL == 1 || LOCAL == 2
inline __device__ void AccumulateLocal(float *lmem, const int loff,
                            const float* coeff, float acc[WPTY][WPTX],
...
#if LOCAL == 0
inline __device__ void AccumulateGlobal(const  float* src, const int goff,
                             const float* coeff, float acc[WPTY][WPTX],
...
inline __device__ void StoreResult( floatvec* dest, const int goff, float acc[WPTY][WPTX],
                        const int gid_x, const int gid_y) {
  #pragma unroll
  for (int wx=0; wx<WPTX/VECTOR; ++wx) {
    const int gx = gid_x*WPTX/VECTOR + wx;
...
#if LOCAL == 0
extern "C" __global__ void conv(const int goff, const int dummy,
                   const  float* src,
                   const float* coeff,
                    floatvec* dest) {

  // Thread identifiers
  const int gid_x = blockDim.x*blockIdx.x + threadIdx.x; // From 0 to goff/WPTX-1
  const int gid_y = blockDim.y*blockIdx.y + threadIdx.y; // From 0 to dummy/WPTY-1
...
extern "C" __global__ void conv(const int goff, const int dummy,
  ...
  // Local memory
  const int lid_x = threadIdx.x; // From 0 to TBX
  const int lid_y = threadIdx.y; // From 0 to TBY
  __shared__ float lmem[(TBY*WPTY + 2*HFS) * (TBX*WPTX + 2*HFS + PADDING)];
...
extern "C" __global__ void conv(const int goff, const int dummy,
  ...
  // Thread identifiers
  const int gid_x = threadIdx.x + TBX*blockIdx.x;
  const int gid_y = threadIdx.y + TBY*blockIdx.y;

  // Local memory
  const int lid_x = threadIdx.x; // From 0 to (TBX + 2*HFS)
  const int lid_y = threadIdx.y; // From 0 to (TBY + 2*HFS)
  __shared__ float lmem[(TBY*WPTY + 2*HFS) * (TBX*WPTX + 2*HFS + PADDING)];
  const int loff = TBX*WPTX + 2*HFS + PADDING;

...
__kernel void directCoulombSum(MEMORY_TYPE_AOS vector* atomInfo, MEMORY_TYPE_SOA vector* atomInfoX, MEMORY_TYPE_SOA vector* atomInfoY,
    MEMORY_TYPE_SOA vector* atomInfoZ, MEMORY_TYPE_SOA vector* atomInfoW, int numberOfAtoms, float gridSpacing, int zIndex, __global float* energyGrid)
{
    int xIndex = get_global_id(0) * OUTER_UNROLL_FACTOR;
    int yIndex = get_global_id(1);
    
    int outIndex = get_global_size(1) * get_global_size(0) * OUTER_UNROLL_FACTOR * zIndex + get_global_size(0) * yIndex * OUTER_UNROLL_FACTOR + xIndex;
    ...
    for (int i = 0; i < numberOfAtoms; i++)
    {
        ...
        float partialResult = native_rsqrt(distanceX * distanceX + distanceY * distanceY + atomInfo[(4 * i) + 2]);
        energyValue += atomInfo[(4 * i) + 3] * partialResult;


        #if OUTER_UNROLL_FACTOR > 1
        float distanceX2 = coordX2 - atomInfo[(4 * i)];
        float partialResult2 = native_rsqrt(distanceX2 * distanceX2 + distanceY * distanceY + atomInfo[(4 * i) + 2]);
        ...
        float partialResult3 = native_rsqrt(distanceX3 * distanceX3 + distanceY * distanceY + atomInfo[(4 * i) + 2]);
        energyValue3 += atomInfo[(4 * i) + 3] * partialResult3;

        float distanceX4 = coordX4 - atomInfo[(4 * i)];
        float partialResult4 = native_rsqrt(distanceX4 * distanceX4 + distanceY * distanceY + atomInfo[(4 * i) + 2]);
        ...
        float partialResult5 = native_rsqrt(distanceX5 * distanceX5 + distanceY * distanceY + atomInfo[(4 * i) + 2]);
        energyValue5 += atomInfo[(4 * i) + 3] * partialResult5;

        float distanceX6 = coordX6 - atomInfo[(4 * i)];
        float partialResult6 = native_rsqrt(distanceX6 * distanceX6 + distanceY * distanceY + atomInfo[(4 * i) + 2]);
        energyValue6 += atomInfo[(4 * i) + 3] * partialResult6;

        float distanceX7 = coordX7 - atomInfo[(4 * i)];
        float partialResult7 = native_rsqrt(distanceX7 * distanceX7 + distanceY * distanceY + atomInfo[(4 * i) + 2]);
        energyValue7 += atomInfo[(4 * i) + 3] * partialResult7;

        float distanceX8 = coordX8 - atomInfo[(4 * i)];
        float partialResult8 = native_rsqrt(distanceX8 * distanceX8 + distanceY * distanceY + atomInfo[(4 * i) + 2]);
        ...
        float partialResult = native_rsqrt(distanceX * distanceX + distanceY * distanceY + atomInfoZ[i]);
        energyValue += atomInfoW[i] * partialResult;


        #if OUTER_UNROLL_FACTOR > 1
        float distanceX2 = coordX2 - atomInfoX[i];
        float partialResult2 = native_rsqrt(distanceX2 * distanceX2 + distanceY * distanceY + atomInfoZ[i]);
        ...
        float partialResult3 = native_rsqrt(distanceX3 * distanceX3 + distanceY * distanceY + atomInfoZ[i]);
        energyValue3 += atomInfoW[i] * partialResult3;

        float distanceX4 = coordX4 - atomInfoX[i];
        float partialResult4 = native_rsqrt(distanceX4 * distanceX4 + distanceY * distanceY + atomInfoZ[i]);
        ...
        float partialResult5 = native_rsqrt(distanceX5 * distanceX5 + distanceY * distanceY + atomInfoZ[i]);
        energyValue5 += atomInfoW[i] * partialResult5;

        float distanceX6 = coordX6 - atomInfoX[i];
        float partialResult6 = native_rsqrt(distanceX6 * distanceX6 + distanceY * distanceY + atomInfoZ[i]);
        energyValue6 += atomInfoW[i] * partialResult6;

        float distanceX7 = coordX7 - atomInfoX[i];
        float partialResult7 = native_rsqrt(distanceX7 * distanceX7 + distanceY * distanceY + atomInfoZ[i]);
        energyValue7 += atomInfoW[i] * partialResult7;

        float distanceX8 = coordX8 - atomInfoX[i];
        float partialResult8 = native_rsqrt(distanceX8 * distanceX8 + distanceY * distanceY + atomInfoZ[i]);
    ...
    for (int i = 0; i < numberOfAtoms / (1 + SOA_UNROLL); i++)
    {
        ...
        float partialResult = native_rsqrt(distanceX * distanceX + distanceY * distanceY + atomInfo[(2 * i) + 1].x);
        energyValue += atomInfo[(2 * i) + 1].y * partialResult;


        #if OUTER_UNROLL_FACTOR > 1
        float distanceX2 = coordX2 - atomInfo[(2 * i)].x;
        float partialResult2 = native_rsqrt(distanceX2 * distanceX2 + distanceY * distanceY + atomInfo[(2 * i) + 1].x);
        ...
        float partialResult3 = native_rsqrt(distanceX3 * distanceX3 + distanceY * distanceY + atomInfo[(2 * i) + 1].x);
        energyValue3 += atomInfo[(2 * i) + 1].y * partialResult3;

        float distanceX4 = coordX4 - atomInfo[(2 * i)].x;
        float partialResult4 = native_rsqrt(distanceX4 * distanceX4 + distanceY * distanceY + atomInfo[(2 * i) + 1].x);
        ...
        float partialResult5 = native_rsqrt(distanceX5 * distanceX5 + distanceY * distanceY + atomInfo[(2 * i) + 1].x);
        energyValue5 += atomInfo[(2 * i) + 1].y * partialResult5;

        float distanceX6 = coordX6 - atomInfo[(2 * i)].x;
        float partialResult6 = native_rsqrt(distanceX6 * distanceX6 + distanceY * distanceY + atomInfo[(2 * i) + 1].x);
        energyValue6 += atomInfo[(2 * i) + 1].y * partialResult6;

        float distanceX7 = coordX7 - atomInfo[(2 * i)].x;
        float partialResult7 = native_rsqrt(distanceX7 * distanceX7 + distanceY * distanceY + atomInfo[(2 * i) + 1].x);
        energyValue7 += atomInfo[(2 * i) + 1].y * partialResult7;

        float distanceX8 = coordX8 - atomInfo[(2 * i)].x;
        float partialResult8 = native_rsqrt(distanceX8 * distanceX8 + distanceY * distanceY + atomInfo[(2 * i) + 1].x);
        ...
        float partialResult = native_rsqrt(distanceX * distanceX + distanceY * distanceY + atomInfoZ[i].x);
        energyValue += atomInfoW[i].x * partialResult;

        float distanceXy = coordX - atomInfoX[i].y;
        float distanceYy = coordY - atomInfoY[i].y;
        float partialResulty = native_rsqrt(distanceXy * distanceXy + distanceYy * distanceYy + atomInfoZ[i].y);
        energyValue += atomInfoW[i].y * partialResulty;


        #if OUTER_UNROLL_FACTOR > 1
        float distanceX2 = coordX2 - atomInfoX[i].x;
        float partialResult2 = native_rsqrt(distanceX2 * distanceX2 + distanceY * distanceY + atomInfoZ[i].x);
        energyValue2 += atomInfoW[i].x * partialResult2;

        float distanceXy2 = coordX2 - atomInfoX[i].y;
        float partialResulty2 = native_rsqrt(distanceXy2 * distanceXy2 + distanceYy * distanceYy + atomInfoZ[i].y);
        ...
        float partialResult3 = native_rsqrt(distanceX3 * distanceX3 + distanceY * distanceY + atomInfoZ[i].x);
        energyValue3 += atomInfoW[i].x * partialResult3;

        float distanceXy3 = coordX3 - atomInfoX[i].y;
        float partialResulty3 = native_rsqrt(distanceXy3 * distanceXy3 + distanceYy * distanceYy + atomInfoZ[i].y);
        energyValue3 += atomInfoW[i].y * partialResulty3;

        float distanceX4 = coordX4 - atomInfoX[i].x;
        float partialResult4 = native_rsqrt(distanceX4 * distanceX4 + distanceY * distanceY + atomInfoZ[i].x);
        energyValue4 += atomInfoW[i].x * partialResult4;

        float distanceXy4 = coordX4 - atomInfoX[i].y;
        float partialResulty4 = native_rsqrt(distanceXy4 * distanceXy4 + distanceYy * distanceYy + atomInfoZ[i].y);
        ...
        float partialResult5 = native_rsqrt(distanceX5 * distanceX5 + distanceY * distanceY + atomInfoZ[i].x);
        energyValue5 += atomInfoW[i].x * partialResult5;

        float distanceXy5 = coordX5 - atomInfoX[i].y;
        float partialResulty5 = native_rsqrt(distanceXy5 * distanceXy5 + distanceYy * distanceYy + atomInfoZ[i].y);
        energyValue5 += atomInfoW[i].y * partialResulty5;

        float distanceX6 = coordX6 - atomInfoX[i].x;
        float partialResult6 = native_rsqrt(distanceX6 * distanceX6 + distanceY * distanceY + atomInfoZ[i].x);
        energyValue6 += atomInfoW[i].x * partialResult6;

        float distanceXy6 = coordX6 - atomInfoX[i].y;
        float partialResulty6 = native_rsqrt(distanceXy6 * distanceXy6 + distanceYy * distanceYy + atomInfoZ[i].y);
        energyValue6 += atomInfoW[i].y * partialResulty6;

        float distanceX7 = coordX7 - atomInfoX[i].x;
        float partialResult7 = native_rsqrt(distanceX7 * distanceX7 + distanceY * distanceY + atomInfoZ[i].x);
        energyValue7 += atomInfoW[i].x * partialResult7;

        float distanceXy7 = coordX7 - atomInfoX[i].y;
        float partialResulty7 = native_rsqrt(distanceXy7 * distanceXy7 + distanceYy * distanceYy + atomInfoZ[i].y);
        energyValue7 += atomInfoW[i].y * partialResulty7;

        float distanceX8 = coordX8 - atomInfoX[i].x;
        float partialResult8 = native_rsqrt(distanceX8 * distanceX8 + distanceY * distanceY + atomInfoZ[i].x);
        energyValue8 += atomInfoW[i].x * partialResult8;

        float distanceXy8 = coordX8 - atomInfoX[i].y;
        float partialResulty8 = native_rsqrt(distanceXy8 * distanceXy8 + distanceYy * distanceYy + atomInfoZ[i].y);
    ...
    for (int i = 0; i < numberOfAtoms / (1 + SOA_UNROLL * 3); i++)
    {
        ...
        float partialResult2 = native_rsqrt(distanceX2 * distanceX2 + distanceY * distanceY + atomInfo[i].z);
        ...
        float partialResult3 = native_rsqrt(distanceX3 * distanceX3 + distanceY * distanceY + atomInfo[i].z);
        energyValue3 += atomInfo[i].w * partialResult3;

        float distanceX4 = coordX4 - atomInfo[i].x;
        float partialResult4 = native_rsqrt(distanceX4 * distanceX4 + distanceY * distanceY + atomInfo[i].z);
        ...
        float partialResult5 = native_rsqrt(distanceX5 * distanceX5 + distanceY * distanceY + atomInfo[i].z);
        energyValue5 += atomInfo[i].w * partialResult5;

        float distanceX6 = coordX6 - atomInfo[i].x;
        float partialResult6 = native_rsqrt(distanceX6 * distanceX6 + distanceY * distanceY + atomInfo[i].z);
        energyValue6 += atomInfo[i].w * partialResult6;

        float distanceX7 = coordX7 - atomInfo[i].x;
        float partialResult7 = native_rsqrt(distanceX7 * distanceX7 + distanceY * distanceY + atomInfo[i].z);
        energyValue7 += atomInfo[i].w * partialResult7;

        float distanceX8 = coordX8 - atomInfo[i].x;
        float partialResult8 = native_rsqrt(distanceX8 * distanceX8 + distanceY * distanceY + atomInfo[i].z);
        ...
        float partialResultz = native_rsqrt(distanceXz * distanceXz + distanceYz * distanceYz + atomInfoZ[i].z);
        energyValue += atomInfoW[i].z * partialResultz;

        float distanceXw = coordX - atomInfoX[i].w;
        float distanceYw = coordY - atomInfoY[i].w;
        float partialResultw = native_rsqrt(distanceXw * distanceXw + distanceYw * distanceYw + atomInfoZ[i].w);
        ...
        float partialResultz2 = native_rsqrt(distanceXz2 * distanceXz2 + distanceYz * distanceYz + atomInfoZ[i].z);
        energyValue2 += atomInfoW[i].z * partialResultz2;

        float distanceXw2 = coordX2 - atomInfoX[i].w;
        float partialResultw2 = native_rsqrt(distanceXw2 * distanceXw2 + distanceYw * distanceYw + atomInfoZ[i].w);
        ...
        float partialResultz3 = native_rsqrt(distanceXz3 * distanceXz3 + distanceYz * distanceYz + atomInfoZ[i].z);
        energyValue3 += atomInfoW[i].z * partialResultz3;

        float distanceXw3 = coordX3 - atomInfoX[i].w;
        float partialResultw3 = native_rsqrt(distanceXw3 * distanceXw3 + distanceYw * distanceYw + atomInfoZ[i].w);
        ...
        float partialResultz4 = native_rsqrt(distanceXz4 * distanceXz4 + distanceYz * distanceYz + atomInfoZ[i].z);
        energyValue4 += atomInfoW[i].z * partialResultz4;

        float distanceXw4 = coordX4 - atomInfoX[i].w;
        float partialResultw4 = native_rsqrt(distanceXw4 * distanceXw4 + distanceYw * distanceYw + atomInfoZ[i].w);
        ...
        float partialResultz5 = native_rsqrt(distanceXz5 * distanceXz5 + distanceYz * distanceYz + atomInfoZ[i].z);
        energyValue5 += atomInfoW[i].z * partialResultz5;

        float distanceXw5 = coordX5 - atomInfoX[i].w;
        float partialResultw5 = native_rsqrt(distanceXw5 * distanceXw5 + distanceYw * distanceYw + atomInfoZ[i].w);
        ...
        float partialResultz6 = native_rsqrt(distanceXz6 * distanceXz6 + distanceYz * distanceYz + atomInfoZ[i].z);
        energyValue6 += atomInfoW[i].z * partialResultz6;

        float distanceXw6 = coordX6 - atomInfoX[i].w;
        float partialResultw6 = native_rsqrt(distanceXw6 * distanceXw6 + distanceYw * distanceYw + atomInfoZ[i].w);
        ...
        float partialResultz7 = native_rsqrt(distanceXz7 * distanceXz7 + distanceYz * distanceYz + atomInfoZ[i].z);
        energyValue7 += atomInfoW[i].z * partialResultz7;

        float distanceXw7 = coordX7 - atomInfoX[i].w;
        float partialResultw7 = native_rsqrt(distanceXw7 * distanceXw7 + distanceYw * distanceYw + atomInfoZ[i].w);
        ...
        float partialResultz8 = native_rsqrt(distanceXz8 * distanceXz8 + distanceYz * distanceYz + atomInfoZ[i].z);
        energyValue8 += atomInfoW[i].z * partialResultz8;

        float distanceXw8 = coordX8 - atomInfoX[i].w;
        float partialResultw8 = native_rsqrt(distanceXw8 * distanceXw8 + distanceYw * distanceYw + atomInfoZ[i].w);
    ...
    for (int i = 0; i < numberOfAtoms / (2 - SOA_UNROLL) / (1 + SOA_UNROLL * 7); i++)
    {
        ...
        float partialResult = native_rsqrt(distanceX * distanceX + distanceY * distanceY + atomInfo[i].s2);
        energyValue += atomInfo[i].s3 * partialResult;

        float distanceX2 = coordX - atomInfo[i].s4;
        float distanceY2 = coordY - atomInfo[i].s5;
        float partialResult2 = native_rsqrt(distanceX2 * distanceX2 + distanceY2 * distanceY2 + atomInfo[i].s6);
        energyValue += atomInfo[i].s7 * partialResult2;


        #if OUTER_UNROLL_FACTOR > 1
        float distanceX21 = coordX2 - atomInfo[i].s0;
        float partialResult21 = native_rsqrt(distanceX21 * distanceX21 + distanceY * distanceY + atomInfo[i].s2);
        energyValue2 += atomInfo[i].s3 * partialResult21;

        float distanceX22 = coordX2 - atomInfo[i].s4;
        float partialResult22 = native_rsqrt(distanceX22 * distanceX22 + distanceY2 * distanceY2 + atomInfo[i].s6);
        ...
        float partialResult31 = native_rsqrt(distanceX31 * distanceX31 + distanceY * distanceY + atomInfo[i].s2);
        energyValue3 += atomInfo[i].s3 * partialResult31;

        float distanceX32 = coordX3 - atomInfo[i].s4;
        float partialResult32 = native_rsqrt(distanceX32 * distanceX32 + distanceY2 * distanceY2 + atomInfo[i].s6);
        energyValue3 += atomInfo[i].s7 * partialResult32;

        float distanceX41 = coordX4 - atomInfo[i].s0;
        float partialResult41 = native_rsqrt(distanceX41 * distanceX41 + distanceY * distanceY + atomInfo[i].s2);
        energyValue4 += atomInfo[i].s3 * partialResult41;

        float distanceX42 = coordX4 - atomInfo[i].s4;
        float partialResult42 = native_rsqrt(distanceX42 * distanceX42 + distanceY2 * distanceY2 + atomInfo[i].s6);
        ...
        float partialResult51 = native_rsqrt(distanceX51 * distanceX51 + distanceY * distanceY + atomInfo[i].s2);
        energyValue5 += atomInfo[i].s3 * partialResult51;

        float distanceX52 = coordX5 - atomInfo[i].s4;
        float partialResult52 = native_rsqrt(distanceX52 * distanceX52 + distanceY2 * distanceY2 + atomInfo[i].s6);
        energyValue5 += atomInfo[i].s7 * partialResult52;

        float distanceX61 = coordX6 - atomInfo[i].s0;
        float partialResult61 = native_rsqrt(distanceX61 * distanceX61 + distanceY * distanceY + atomInfo[i].s2);
        energyValue6 += atomInfo[i].s3 * partialResult61;

        float distanceX62 = coordX6 - atomInfo[i].s4;
        float partialResult62 = native_rsqrt(distanceX62 * distanceX62 + distanceY2 * distanceY2 + atomInfo[i].s6);
        energyValue6 += atomInfo[i].s7 * partialResult62;

        float distanceX71 = coordX7 - atomInfo[i].s0;
        float partialResult71 = native_rsqrt(distanceX71 * distanceX71 + distanceY * distanceY + atomInfo[i].s2);
        energyValue7 += atomInfo[i].s3 * partialResult71;

        float distanceX72 = coordX7 - atomInfo[i].s4;
        float partialResult72 = native_rsqrt(distanceX72 * distanceX72 + distanceY2 * distanceY2 + atomInfo[i].s6);
        energyValue7 += atomInfo[i].s7 * partialResult72;

        float distanceX81 = coordX8 - atomInfo[i].s0;
        float partialResult81 = native_rsqrt(distanceX81 * distanceX81 + distanceY * distanceY + atomInfo[i].s2);
        energyValue8 += atomInfo[i].s3 * partialResult81;

        float distanceX82 = coordX8 - atomInfo[i].s4;
        float partialResult82 = native_rsqrt(distanceX82 * distanceX82 + distanceY2 * distanceY2 + atomInfo[i].s6);
        ...
        float partialResults0 = native_rsqrt(distanceXs0 * distanceXs0 + distanceYs0 * distanceYs0 + atomInfoZ[i].s0);
        energyValue += atomInfoW[i].s0 * partialResults0;

        float distanceXs1 = coordX - atomInfoX[i].s1;
        float distanceYs1 = coordY - atomInfoY[i].s1;
        float partialResults1 = native_rsqrt(distanceXs1 * distanceXs1 + distanceYs1 * distanceYs1 + atomInfoZ[i].s1);
        energyValue += atomInfoW[i].s1 * partialResults1;

        float distanceXs2 = coordX - atomInfoX[i].s2;
        float distanceYs2 = coordY - atomInfoY[i].s2;
        float partialResults2 = native_rsqrt(distanceXs2 * distanceXs2 + distanceYs2 * distanceYs2 + atomInfoZ[i].s2);
        energyValue += atomInfoW[i].s2 * partialResults2;

        float distanceXs3 = coordX - atomInfoX[i].s3;
        float distanceYs3 = coordY - atomInfoY[i].s3;
        float partialResults3 = native_rsqrt(distanceXs3 * distanceXs3 + distanceYs3 * distanceYs3 + atomInfoZ[i].s3);
        energyValue += atomInfoW[i].s3 * partialResults3;

        float distanceXs4 = coordX - atomInfoX[i].s4;
        float distanceYs4 = coordY - atomInfoY[i].s4;
        float partialResults4 = native_rsqrt(distanceXs4 * distanceXs4 + distanceYs4 * distanceYs4 + atomInfoZ[i].s4);
        energyValue += atomInfoW[i].s4 * partialResults4;

        float distanceXs5 = coordX - atomInfoX[i].s5;
        float distanceYs5 = coordY - atomInfoY[i].s5;
        float partialResults5 = native_rsqrt(distanceXs5 * distanceXs5 + distanceYs5 * distanceYs5 + atomInfoZ[i].s5);
        energyValue += atomInfoW[i].s5 * partialResults5;

        float distanceXs6 = coordX - atomInfoX[i].s6;
        float distanceYs6 = coordY - atomInfoY[i].s6;
        float partialResults6 = native_rsqrt(distanceXs6 * distanceXs6 + distanceYs6 * distanceYs6 + atomInfoZ[i].s6);
        energyValue += atomInfoW[i].s6 * partialResults6;

        float distanceXs7 = coordX - atomInfoX[i].s7;
        float distanceYs7 = coordY - atomInfoY[i].s7;
        float partialResults7 = native_rsqrt(distanceXs7 * distanceXs7 + distanceYs7 * distanceYs7 + atomInfoZ[i].s7);
        energyValue += atomInfoW[i].s7 * partialResults7;


        #if OUTER_UNROLL_FACTOR > 1
        float distanceXs02 = coordX2 - atomInfoX[i].s0;
        float partialResults02 = native_rsqrt(distanceXs02 * distanceXs02 + distanceYs0 * distanceYs0 + atomInfoZ[i].s0);
        energyValue2 += atomInfoW[i].s0 * partialResults02;

        float distanceXs12 = coordX2 - atomInfoX[i].s1;
        float partialResults12 = native_rsqrt(distanceXs12 * distanceXs12 + distanceYs1 * distanceYs1 + atomInfoZ[i].s1);
        energyValue2 += atomInfoW[i].s1 * partialResults12;

        float distanceXs22 = coordX2 - atomInfoX[i].s2;
        float partialResults22 = native_rsqrt(distanceXs22 * distanceXs22 + distanceYs2 * distanceYs2 + atomInfoZ[i].s2);
        energyValue2 += atomInfoW[i].s2 * partialResults22;

        float distanceXs32 = coordX2 - atomInfoX[i].s3;
        float partialResults32 = native_rsqrt(distanceXs32 * distanceXs32 + distanceYs3 * distanceYs3 + atomInfoZ[i].s3);
        energyValue2 += atomInfoW[i].s3 * partialResults32;

        float distanceXs42 = coordX2 - atomInfoX[i].s4;
        float partialResults42 = native_rsqrt(distanceXs42 * distanceXs42 + distanceYs4 * distanceYs4 + atomInfoZ[i].s4);
        energyValue2 += atomInfoW[i].s4 * partialResults42;

        float distanceXs52 = coordX2 - atomInfoX[i].s5;
        float partialResults52 = native_rsqrt(distanceXs52 * distanceXs52 + distanceYs5 * distanceYs5 + atomInfoZ[i].s5);
        energyValue2 += atomInfoW[i].s5 * partialResults52;

        float distanceXs62 = coordX2 - atomInfoX[i].s6;
        float partialResults62 = native_rsqrt(distanceXs62 * distanceXs62 + distanceYs6 * distanceYs6 + atomInfoZ[i].s6);
        energyValue2 += atomInfoW[i].s6 * partialResults62;

        float distanceXs72 = coordX2 - atomInfoX[i].s7;
        float partialResults72 = native_rsqrt(distanceXs72 * distanceXs72 + distanceYs7 * distanceYs7 + atomInfoZ[i].s7);
        ...
        float partialResults03 = native_rsqrt(distanceXs03 * distanceXs03 + distanceYs0 * distanceYs0 + atomInfoZ[i].s0);
        energyValue3 += atomInfoW[i].s0 * partialResults03;

        float distanceXs13 = coordX3 - atomInfoX[i].s1;
        float partialResults13 = native_rsqrt(distanceXs13 * distanceXs13 + distanceYs1 * distanceYs1 + atomInfoZ[i].s1);
        energyValue3 += atomInfoW[i].s1 * partialResults13;

        float distanceXs23 = coordX3 - atomInfoX[i].s2;
        float partialResults23 = native_rsqrt(distanceXs23 * distanceXs23 + distanceYs2 * distanceYs2 + atomInfoZ[i].s2);
        energyValue3 += atomInfoW[i].s2 * partialResults23;

        float distanceXs33 = coordX3 - atomInfoX[i].s3;
        float partialResults33 = native_rsqrt(distanceXs33 * distanceXs33 + distanceYs3 * distanceYs3 + atomInfoZ[i].s3);
        energyValue3 += atomInfoW[i].s3 * partialResults33;

        float distanceXs43 = coordX3 - atomInfoX[i].s4;
        float partialResults43 = native_rsqrt(distanceXs43 * distanceXs43 + distanceYs4 * distanceYs4 + atomInfoZ[i].s4);
        energyValue3 += atomInfoW[i].s4 * partialResults43;

        float distanceXs53 = coordX3 - atomInfoX[i].s5;
        float partialResults53 = native_rsqrt(distanceXs53 * distanceXs53 + distanceYs5 * distanceYs5 + atomInfoZ[i].s5);
        energyValue3 += atomInfoW[i].s5 * partialResults53;

        float distanceXs63 = coordX3 - atomInfoX[i].s6;
        float partialResults63 = native_rsqrt(distanceXs63 * distanceXs63 + distanceYs6 * distanceYs6 + atomInfoZ[i].s6);
        energyValue3 += atomInfoW[i].s6 * partialResults63;

        float distanceXs73 = coordX3 - atomInfoX[i].s7;
        float partialResults73 = native_rsqrt(distanceXs73 * distanceXs73 + distanceYs7 * distanceYs7 + atomInfoZ[i].s7);
        energyValue3 += atomInfoW[i].s7 * partialResults73;

        float distanceXs04 = coordX4 - atomInfoX[i].s0;
        float partialResults04 = native_rsqrt(distanceXs04 * distanceXs04 + distanceYs0 * distanceYs0 + atomInfoZ[i].s0);
        energyValue4 += atomInfoW[i].s0 * partialResults04;

        float distanceXs14 = coordX4 - atomInfoX[i].s1;
        float partialResults14 = native_rsqrt(distanceXs14 * distanceXs14 + distanceYs1 * distanceYs1 + atomInfoZ[i].s1);
        energyValue4 += atomInfoW[i].s1 * partialResults14;

        float distanceXs24 = coordX4 - atomInfoX[i].s2;
        float partialResults24 = native_rsqrt(distanceXs24 * distanceXs24 + distanceYs2 * distanceYs2 + atomInfoZ[i].s2);
        energyValue4 += atomInfoW[i].s2 * partialResults24;

        float distanceXs34 = coordX4 - atomInfoX[i].s3;
        float partialResults34 = native_rsqrt(distanceXs34 * distanceXs34 + distanceYs3 * distanceYs3 + atomInfoZ[i].s3);
        energyValue4 += atomInfoW[i].s3 * partialResults34;

        float distanceXs44 = coordX4 - atomInfoX[i].s4;
        float partialResults44 = native_rsqrt(distanceXs44 * distanceXs44 + distanceYs4 * distanceYs4 + atomInfoZ[i].s4);
        energyValue4 += atomInfoW[i].s4 * partialResults44;

        float distanceXs54 = coordX4 - atomInfoX[i].s5;
        float partialResults54 = native_rsqrt(distanceXs54 * distanceXs54 + distanceYs5 * distanceYs5 + atomInfoZ[i].s5);
        energyValue4 += atomInfoW[i].s5 * partialResults54;

        float distanceXs64 = coordX4 - atomInfoX[i].s6;
        float partialResults64 = native_rsqrt(distanceXs64 * distanceXs64 + distanceYs6 * distanceYs6 + atomInfoZ[i].s6);
        energyValue4 += atomInfoW[i].s6 * partialResults64;

        float distanceXs74 = coordX4 - atomInfoX[i].s7;
        float partialResults74 = native_rsqrt(distanceXs74 * distanceXs74 + distanceYs7 * distanceYs7 + atomInfoZ[i].s7);
        ...
        float partialResults05 = native_rsqrt(distanceXs05 * distanceXs05 + distanceYs0 * distanceYs0 + atomInfoZ[i].s0);
        energyValue5 += atomInfoW[i].s0 * partialResults05;

        float distanceXs15 = coordX5 - atomInfoX[i].s1;
        float partialResults15 = native_rsqrt(distanceXs15 * distanceXs15 + distanceYs1 * distanceYs1 + atomInfoZ[i].s1);
        energyValue5 += atomInfoW[i].s1 * partialResults15;

        float distanceXs25 = coordX5 - atomInfoX[i].s2;
        float partialResults25 = native_rsqrt(distanceXs25 * distanceXs25 + distanceYs2 * distanceYs2 + atomInfoZ[i].s2);
        energyValue5 += atomInfoW[i].s2 * partialResults25;

        float distanceXs35 = coordX5 - atomInfoX[i].s3;
        float partialResults35 = native_rsqrt(distanceXs35 * distanceXs35 + distanceYs3 * distanceYs3 + atomInfoZ[i].s3);
        energyValue5 += atomInfoW[i].s3 * partialResults35;

        float distanceXs45 = coordX5 - atomInfoX[i].s4;
        float partialResults45 = native_rsqrt(distanceXs45 * distanceXs45 + distanceYs4 * distanceYs4 + atomInfoZ[i].s4);
        energyValue5 += atomInfoW[i].s4 * partialResults45;

        float distanceXs55 = coordX5 - atomInfoX[i].s5;
        float partialResults55 = native_rsqrt(distanceXs55 * distanceXs55 + distanceYs5 * distanceYs5 + atomInfoZ[i].s5);
        energyValue5 += atomInfoW[i].s5 * partialResults55;

        float distanceXs65 = coordX5 - atomInfoX[i].s6;
        float partialResults65 = native_rsqrt(distanceXs65 * distanceXs65 + distanceYs6 * distanceYs6 + atomInfoZ[i].s6);
        energyValue5 += atomInfoW[i].s6 * partialResults65;

        float distanceXs75 = coordX5 - atomInfoX[i].s7;
        float partialResults75 = native_rsqrt(distanceXs75 * distanceXs75 + distanceYs7 * distanceYs7 + atomInfoZ[i].s7);
        energyValue5 += atomInfoW[i].s7 * partialResults75;

        float distanceXs06 = coordX6 - atomInfoX[i].s0;
        float partialResults06 = native_rsqrt(distanceXs06 * distanceXs06 + distanceYs0 * distanceYs0 + atomInfoZ[i].s0);
        energyValue6 += atomInfoW[i].s0 * partialResults06;

        float distanceXs16 = coordX6 - atomInfoX[i].s1;
        float partialResults16 = native_rsqrt(distanceXs16 * distanceXs16 + distanceYs1 * distanceYs1 + atomInfoZ[i].s1);
        energyValue6 += atomInfoW[i].s1 * partialResults16;

        float distanceXs26 = coordX6 - atomInfoX[i].s2;
        float partialResults26 = native_rsqrt(distanceXs26 * distanceXs26 + distanceYs2 * distanceYs2 + atomInfoZ[i].s2);
        energyValue6 += atomInfoW[i].s2 * partialResults26;

        float distanceXs36 = coordX6 - atomInfoX[i].s3;
        float partialResults36 = native_rsqrt(distanceXs36 * distanceXs36 + distanceYs3 * distanceYs3 + atomInfoZ[i].s3);
        energyValue6 += atomInfoW[i].s3 * partialResults36;

        float distanceXs46 = coordX6 - atomInfoX[i].s4;
        float partialResults46 = native_rsqrt(distanceXs46 * distanceXs46 + distanceYs4 * distanceYs4 + atomInfoZ[i].s4);
        energyValue6 += atomInfoW[i].s4 * partialResults46;

        float distanceXs56 = coordX6 - atomInfoX[i].s5;
        float partialResults56 = native_rsqrt(distanceXs56 * distanceXs56 + distanceYs5 * distanceYs5 + atomInfoZ[i].s5);
        energyValue6 += atomInfoW[i].s5 * partialResults56;

        float distanceXs66 = coordX6 - atomInfoX[i].s6;
        float partialResults66 = native_rsqrt(distanceXs66 * distanceXs66 + distanceYs6 * distanceYs6 + atomInfoZ[i].s6);
        energyValue6 += atomInfoW[i].s6 * partialResults66;

        float distanceXs76 = coordX6 - atomInfoX[i].s7;
        float partialResults76 = native_rsqrt(distanceXs76 * distanceXs76 + distanceYs7 * distanceYs7 + atomInfoZ[i].s7);
        energyValue6 += atomInfoW[i].s7 * partialResults76;

        float distanceXs07 = coordX7 - atomInfoX[i].s0;
        float partialResults07 = native_rsqrt(distanceXs07 * distanceXs07 + distanceYs0 * distanceYs0 + atomInfoZ[i].s0);
        energyValue7 += atomInfoW[i].s0 * partialResults07;

        float distanceXs17 = coordX7 - atomInfoX[i].s1;
        float partialResults17 = native_rsqrt(distanceXs17 * distanceXs17 + distanceYs1 * distanceYs1 + atomInfoZ[i].s1);
        energyValue7 += atomInfoW[i].s1 * partialResults17;

        float distanceXs27 = coordX7 - atomInfoX[i].s2;
        float partialResults27 = native_rsqrt(distanceXs27 * distanceXs27 + distanceYs2 * distanceYs2 + atomInfoZ[i].s2);
        energyValue7 += atomInfoW[i].s2 * partialResults27;

        float distanceXs37 = coordX7 - atomInfoX[i].s3;
        float partialResults37 = native_rsqrt(distanceXs37 * distanceXs37 + distanceYs3 * distanceYs3 + atomInfoZ[i].s3);
        energyValue7 += atomInfoW[i].s3 * partialResults37;

        float distanceXs47 = coordX7 - atomInfoX[i].s4;
        float partialResults47 = native_rsqrt(distanceXs47 * distanceXs47 + distanceYs4 * distanceYs4 + atomInfoZ[i].s4);
        energyValue7 += atomInfoW[i].s4 * partialResults47;

        float distanceXs57 = coordX7 - atomInfoX[i].s5;
        float partialResults57 = native_rsqrt(distanceXs57 * distanceXs57 + distanceYs5 * distanceYs5 + atomInfoZ[i].s5);
        energyValue7 += atomInfoW[i].s5 * partialResults57;

        float distanceXs67 = coordX7 - atomInfoX[i].s6;
        float partialResults67 = native_rsqrt(distanceXs67 * distanceXs67 + distanceYs6 * distanceYs6 + atomInfoZ[i].s6);
        energyValue7 += atomInfoW[i].s6 * partialResults67;

        float distanceXs77 = coordX7 - atomInfoX[i].s7;
        float partialResults77 = native_rsqrt(distanceXs77 * distanceXs77 + distanceYs7 * distanceYs7 + atomInfoZ[i].s7);
        energyValue7 += atomInfoW[i].s7 * partialResults77;

        float distanceXs08 = coordX8 - atomInfoX[i].s0;
        float partialResults08 = native_rsqrt(distanceXs08 * distanceXs08 + distanceYs0 * distanceYs0 + atomInfoZ[i].s0);
        energyValue8 += atomInfoW[i].s0 * partialResults08;

        float distanceXs18 = coordX8 - atomInfoX[i].s1;
        float partialResults18 = native_rsqrt(distanceXs18 * distanceXs18 + distanceYs1 * distanceYs1 + atomInfoZ[i].s1);
        energyValue8 += atomInfoW[i].s1 * partialResults18;

        float distanceXs28 = coordX8 - atomInfoX[i].s2;
        float partialResults28 = native_rsqrt(distanceXs28 * distanceXs28 + distanceYs2 * distanceYs2 + atomInfoZ[i].s2);
        energyValue8 += atomInfoW[i].s2 * partialResults28;

        float distanceXs38 = coordX8 - atomInfoX[i].s3;
        float partialResults38 = native_rsqrt(distanceXs38 * distanceXs38 + distanceYs3 * distanceYs3 + atomInfoZ[i].s3);
        energyValue8 += atomInfoW[i].s3 * partialResults38;

        float distanceXs48 = coordX8 - atomInfoX[i].s4;
        float partialResults48 = native_rsqrt(distanceXs48 * distanceXs48 + distanceYs4 * distanceYs4 + atomInfoZ[i].s4);
        energyValue8 += atomInfoW[i].s4 * partialResults48;

        float distanceXs58 = coordX8 - atomInfoX[i].s5;
        float partialResults58 = native_rsqrt(distanceXs58 * distanceXs58 + distanceYs5 * distanceYs5 + atomInfoZ[i].s5);
        energyValue8 += atomInfoW[i].s5 * partialResults58;

        float distanceXs68 = coordX8 - atomInfoX[i].s6;
        float partialResults68 = native_rsqrt(distanceXs68 * distanceXs68 + distanceYs6 * distanceYs6 + atomInfoZ[i].s6);
        energyValue8 += atomInfoW[i].s6 * partialResults68;

        float distanceXs78 = coordX8 - atomInfoX[i].s7;
        float partialResults78 = native_rsqrt(distanceXs78 * distanceXs78 + distanceYs7 * distanceYs7 + atomInfoZ[i].s7);
    ...
    for (int i = 0; i < numberOfAtoms / VECTOR_TYPE; i++)
    {
        ...
        vector partialResult = native_rsqrt(distanceX * distanceX + distanceY * distanceY + atomInfoZ[i]);
        energyValueVect += atomInfoW[i] * partialResult;


        #if OUTER_UNROLL_FACTOR > 1
        vector distanceX2 = coordX2 - atomInfoX[i];
        vector partialResult2 = native_rsqrt(distanceX2 * distanceX2 + distanceY * distanceY + atomInfoZ[i]);
        ...
        vector partialResult3 = native_rsqrt(distanceX3 * distanceX3 + distanceY * distanceY + atomInfoZ[i]);
        energyValueVect3 += atomInfoW[i] * partialResult3;

        vector distanceX4 = coordX4 - atomInfoX[i];
        vector partialResult4 = native_rsqrt(distanceX4 * distanceX4 + distanceY * distanceY + atomInfoZ[i]);
        ...
        vector partialResult5 = native_rsqrt(distanceX5 * distanceX5 + distanceY * distanceY + atomInfoZ[i]);
        energyValueVect5 += atomInfoW[i] * partialResult5;

        vector distanceX6 = coordX6 - atomInfoX[i];
        vector partialResult6 = native_rsqrt(distanceX6 * distanceX6 + distanceY * distanceY + atomInfoZ[i]);
        energyValueVect6 += atomInfoW[i] * partialResult6;

        vector distanceX7 = coordX7 - atomInfoX[i];
        vector partialResult7 = native_rsqrt(distanceX7 * distanceX7 + distanceY * distanceY + atomInfoZ[i]);
        energyValueVect7 += atomInfoW[i] * partialResult7;

        vector distanceX8 = coordX8 - atomInfoX[i];
        vector partialResult8 = native_rsqrt(distanceX8 * distanceX8 + distanceY * distanceY + atomInfoZ[i]);

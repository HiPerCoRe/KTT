...
"# BATCH: number of configuration from which the fastest one is profiled, set in KTT\n" +
"# NEIGHBOR_SIZE: number of neighboring configurations that are used for batch selection, set in KTT\n" +
"# RANDOM_SIZE: number of random configurations that are used for batch selection, set in KTT\n" +
...
"def randomSearchStep(rng, tuningSpaceSize) :\n" +
"    return int(rng.random() * tuningSpaceSize)\n" +
"\n" +
"# weightedRandomSearchStep\n" +
"# perform one step of random search using weighted probability based on\n" +
"# profiling counters\n" +
"def weightedRandomSearchStep(rng, scoreDistrib, tuningSpaceSize) :\n" +
"    if (sum(scoreDistrib) == 0.0) :\n" +
"        print(\"Weighted search error: no more tuning configurations.\")\n" +
"        return randomSearchStep(rng, tuningSpaceSize)\n" +
"\n" +
"    rnd = rng.random() * sum(scoreDistrib)\n" +
"    idx = 0\n" +
"    tmp = 0.0\n" +
...
"    randomSize = -1\n" +
"    verbose = 0\n" +
"    # private random number generator, seeded in Configure\n" +
"    rng = random.Random()\n" +
...
"\n" +
"    def Configure(self, tuner, modelFile, batchSize, neighborSize, randomSize, logLevel, seed = None):\n" +
"        self.tuner = tuner\n" +
"        # a private generator is used instead of the global one, so that the searcher is not affected\n" +
"        # by random numbers drawn elsewhere in the process\n" +
...
"            print(\"[Profile-based searcher info] Seed set to\", seed)\n" +
"\n" +
"\n" +
...
"            # initialize the batch, make sure it includes unique, i.e. non-repeating configurations\n" +
"            count = 0\n" +
"            maxBatchSize = min(self.batchSize, self.GetUnexploredConfigurationsCount())\n" +
...
"                candidates = self.GetNeighbourConfigurations(self.bestConf, NEIGHBOR_DISTANCE, self.neighborSize)\n" +
...
"                maxPossibleCandidatesSize = min(len(candidates) + self.randomSize, self.GetUnexploredConfigurationsCount())\n" +
...
"                # select next batch\n" +
"                selectedIndices = []\n" +
"                # if we have more candidates than batchSize, use weightedRandom to choose from them, biasing with score\n" +
"                if len(candidates) > self.batchSize :\n" +
"                    numInBatch = 0\n" +
"                    while numInBatch < self.batchSize :\n" +
"                        idx = weightedRandomSearchStep(self.rng, scoreDistrib, len(candidates))\n" +
...
"                # if we have less candidates than batchSize, just put them all in batch\n" +
"                else:\n" +
...
"            return defaultSM\n" +
"\n" +
"def executeSearcher(tuner, kernel, model, batchSize, neighborSize, randomSize, logLevel, seed = None):\n" +
"    searcher = PyProfilingSearcher()\n" +
"    tuner.SetSearcher(kernel, searcher)\n" +
"    searcher.Configure(tuner, model, batchSize, neighborSize, randomSize, logLevel, seed)\n" +
"";
} // namespace ktt

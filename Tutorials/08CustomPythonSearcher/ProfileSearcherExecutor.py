...
# This example demonstrates how to create a more complex custom searcher.
# The searcher is based on an old version of a `ProfileBasedSearcher`.
# Make sure to compile KTT with `--profiling=cupti[-legacy] --python` and put
# compiled `pyktt.so` into this folder.
...
class PyProfilingSearcher(ktt.Searcher):
    ...
    def Configure(self, tuner, modelFile):
        self.tuner = tuner
        self.ccMajor = (
            tuner.GetCurrentDeviceInfo().GetCUDAComputeCapabilityMajor()
        )
        self.ccMinor = (
            tuner.GetCurrentDeviceInfo().GetCUDAComputeCapabilityMinor()
        )
    ...
    def CalculateNextConfiguration(self, previousResult):
        ...
        if len(self.preselectedBatch) > 0:
            ...
            if (self.bestConf == None) or (
                previousResult.GetKernelDuration() < self.bestDuration
            ):
            ...
            if self.bestDuration != -1:
                ...
                if len(previousResult.GetResults()) > 1:
                    print(
                        'Warning: this version of profile-based searcher does not support searching kernels collections. Using counters from kernels 0 only.'
                    )
                globalSize = previousResult.GetResults()[0].GetGlobalSize()
                localSize = previousResult.GetResults()[0].GetLocalSize()
                profilingCountersRun = (
                    previousResult.GetResults()[0]
                    .GetProfilingData()
                    .GetCounters()
                )  # FIXME this supposes there is no composition profiled
                ...
                for pd in profilingCountersRun:
                    ...
                    if pd.GetType() == ktt.ProfilingCounterType.Int:
                        ...
                        (pd.GetType() == ktt.ProfilingCounterType.UnsignedInt)
                        or (pd.GetType() == ktt.ProfilingCounterType.Throughput)
                        or (
                            pd.GetType()
                    ...
                    elif (pd.GetType() == ktt.ProfilingCounterType.Double) or (
                        pd.GetType() == ktt.ProfilingCounterType.Percent
                    ):
                        pcVals.append(pd.GetValueDouble())
                    else:
                        print(
                            'Fatal error, unsupported PC value passed to profile-based searcher!'
                        )
                        exit(1)

                # select candidate configurations according to position of the best one plus some random sample
                candidates = self.GetNeighbourConfigurations(
                    self.bestConf, 2, 100
                )
                for i in range(0, 10):
                    candidates.append(self.GetRandomConfiguration())
                print(
                    'Profile-based searcher: evaluating model for '
                    + str(len(candidates))
                    + ' candidates...'
                )
                ...
                bottlenecks = analyzeBottlenecks(
                    ...
                    self.convertSM2Cores() * self.multiprocessors,
                )
                changes = computeChanges(
                    bottlenecks, self.profilingCountersModel, self.cc
                )
                scoreDistrib = scoreTuningConfigurationsPredictor(
                    changes,
                    self.tuningParamsNames,
                    myTuningSpace,
                    candidatesTuningSpace,
                    scoreDistrib,
                    self.model,
                ...
                # select next batch
                for i in range(0, BATCH):
                    idx = weightedRandomSearchStep(
                        scoreDistrib, len(candidates)
                    )
...
def main():
    ...
    searcher.Configure(tuner, 'Models/1070-coulomb_output_DT.sav')

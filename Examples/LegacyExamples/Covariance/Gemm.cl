// *************************************************************************************************
// The following kernel is a slight modification of the GEMM tunable example in CLTune project, so
// it can be used in Covariance problem. The original notice follows.
// *************************************************************************************************
//
// =================================================================================================
...
#if SA == 1
inline void GlobalToLocalA(const __global realM* restrict agm, __local realM* alm, const int kSizeM,
...
#if SB == 1
inline void GlobalToLocalB(const __global realN* restrict bgm, __local realN* blm, const int kSizeN,
    const int tid, const int kwg) {
...
// Same as above, but now for the B input matrix
#if SB == 0
inline void GlobalToPrivateB(
    const __global realN* restrict bgm, realN bpm[NWI / VWN], const int kSizeN, const int idk) {
#  pragma unroll
...
#pragma unroll
    for (int mi = 0; mi < MWI / VWM; ++mi) {
      ...
      cgm[index] = cpm[ni][mi];
#if SYM_STORE == 1
      cgm[idm * (kSizeM / VWM) + idn] = cpm[ni][mi];
#endif
    }
  }
}
...
inline void MultiplyAccumulate(
    realM cpm[NWI][MWI / VWM], realM apm[MWI / VWM], realN bpm[NWI / VWN]) {
#pragma unroll
  for (int ni = 0; ni < NWI / VWN; ++ni) {
#pragma unroll
    for (int mi = 0; mi < MWI / VWM; ++mi) {
...
#if SB == 1
    GlobalToLocalB(agm, blm, kSizeM, tid, kwg);
#endif
...
#else
        GlobalToPrivateB(agm, bpm, kSizeM, idk);
#endif

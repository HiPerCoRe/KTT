...
extern "C" __global__ void hotspot(int iteration,
  ...
  __shared__ float temp_on_cuda[BLOCK_SIZE_ROWS][BLOCK_SIZE_COLS];
#if LOCAL_MEMORY
  __shared__ float power_on_cuda[BLOCK_SIZE_ROWS][BLOCK_SIZE_COLS];
#endif
  __shared__ float temp_t[BLOCK_SIZE_ROWS][BLOCK_SIZE_COLS]; // saving temporary temperature result
